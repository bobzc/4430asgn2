...
void milestone_2(int fd){
	...
	conn_flag = is_conn_alive(buf, buf + total_count);

	/* get host address */
	get_host(buf, buf + total_count, &host_addr);
	...
	send(server_fd, buf, total_count, 0);
	
	count = recv(server_fd, buf, HEADER_BUFFER_SIZE, 0);
	write(fd, buf, count);
}
